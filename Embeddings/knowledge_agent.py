...
class MultiModelRAGSystem:
    ...
    def __init__(
        ...
        for idx, row in self.df.iterrows():
            # Combine title and overview for richer embedding
            chunk_text = f"{row['title']}\n{row['overview']}"
    ...
    def ai_analyze_chunks(
        ...
        for i, chunk in enumerate(chunks):
            ...
            chunks_text += f"Overview: {chunk['overview']}\n"
    ...
    def process_query(self, query: str, k: int = 5) -> Dict:
        ...
        for model_idx in range(3):
            ...
            for chunk in chunks:
                print(f"  - {chunk['title'][:60]}... (score: {chunk['score']:.4f})")
            print()
            ...
            if analysis['useful']:
                # Found useful chunk!
                print(f"✓ Useful SOP found with model {model_num}!")
                selected_chunk = analysis['selected_chunk']
                print(f"  Selected: {selected_chunk['title']}\n")
                ...
                # NEW FORMAT - Updated print statements
                ...
                return {
                  ...
                  'message': f'Relevant SOP found using embedding model {model_num}'
                }
            else:
                print(f"✗ No useful SOP found with model {model_num}")
                print(f"  Reasoning: {analysis['reasoning'][:200]}...\n")

        # All models failed
        print(f"{'='*80}")
        print("NO RELEVANT SOP FOUND AFTER TRYING ALL 3 MODELS")
        print(f"{'='*80}\n")
        ...
        return {
          ...
          'model_used': None,
          'cleaned_query': cleaned_query,
          'message': 'No relevant SOP found after trying all 3 embedding models'
        }
...
def create_rag_api_handler(rag_system: MultiModelRAGSystem):
...
def batch_process_queries(rag_system: MultiModelRAGSystem, queries: List[str]) -> List[Dict]:
...
def knowledge_model(query):
    """Example usage with error handling."""

    try:
        # Initialize system
        rag_system = MultiModelRAGSystem(
            csv_path='knowledge_base_chunks_combined.csv',
            embedding_models=[
                'sentence-transformers/all-MiniLM-L6-v2',
                'sentence-transformers/all-mpnet-base-v2',
                'sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2'
        ...
        # Process query
        result = rag_system.process_query(query, k=5)
